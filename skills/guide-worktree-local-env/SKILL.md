...
For example, a project that originally uses ports 3000, 5432, and 5173 would be reconfigured to use 8000, 8001, and 8002 in the main worktree — and 8010, 8011, and 8012 in worktree slot 8010. This is a one-time migration of the project's dev environment configuration.
...
**Design the port scheme based on how many ports each environment needs.** The template script uses a step of 10 between slots, which leaves room for multiple ports per environment (e.g. server=8010, frontend=8011). Some examples:

- A project with a server port and a frontend port could use slots 8010–8090 (step of 10), assigning e.g. server=8010, frontend=8011.
- A project with server, database, and frontend ports could also use a step of 10, assigning e.g. server=8010, db=8012, frontend=8011.
- A project with only a single port could simplify to a step of 1 (e.g. slots 5001–5009). In that case, remove the `PORT_STEP` constant, the modulo check in `isValidPort()`, and the secondary port derivation (`adminUiPort`) from the template script.

The slot is identified by the primary port number itself (e.g., `--slot 8020`).
...
{
  "slots": {
    "8010": { "worktree": "/absolute/path/to/myproject-feat-214", "branch": "feat/214" },
    "8030": { "worktree": "/absolute/path/to/myproject-feat-234", "branch": "feat/234" }
  }
}
```
...
| `--free`      | Release the slot assigned to this worktree (cleanup)          |
...
3. Spawns the dev command as a detached process group, redirecting stdout/stderr to a log file.
4. Polls the log file for a success marker string (e.g., `"Server is ready on port"`).
5. Writes the PID to a file. On `--stop`, reads the PID and kills the process group.
...
- For Docker Compose projects, you might run `docker compose up -d` and poll a health endpoint instead of reading a log file.
...
- [ ] **Design your port scheme.** How many ports per environment? What's the base port? What's the step between slots?
